...
// EmailVerificationRepository defines the interface for email verification token data access.
type EmailVerificationRepository interface {
	Create(ctx context.Context, token *model.EmailVerificationToken) error
	GetActiveForUser(ctx context.Context, userID string) (*model.EmailVerificationToken, error)

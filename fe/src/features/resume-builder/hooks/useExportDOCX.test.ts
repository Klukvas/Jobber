...
describe("useExportDOCX", () => {
  ...
  it("calls postBlob with the correct endpoint and timeout", async () => {
    ...
    const { result } = renderHook(() => useExportDOCX(), {
    ...
    expect(apiClient.postBlob).toHaveBeenCalledWith(
      "resume-builder/resume-1/export-docx",
      undefined,
      60000,
    );
  });
  ...
  it("uses the resume ID in the URL path", async () => {
    ...
    await act(async () => {
      result.current.mutate("xyz-789");
    });

    await waitFor(() => expect(result.current.isSuccess).toBe(true));

    expect(apiClient.postBlob).toHaveBeenCalledWith(
      "resume-builder/xyz-789/export-docx",
      undefined,
      60000,
    );
  });

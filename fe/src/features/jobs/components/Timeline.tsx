...
export function Timeline({ stages, jobId, stageComments }: TimelineProps) {
  const { t } = useTranslation();
  const dateLocale = useDateLocale();
  const queryClient = useQueryClient();
  const [selectedStage, setSelectedStage] = useState<JobStageDTO | null>(null);
  const [menuOpen, setMenuOpen] = useState<string | null>(null);
  const [confirmDeleteStage, setConfirmDeleteStage] =
    useState<JobStageDTO | null>(null);
  ...
  const completeStage = useMutation({
    mutationFn: (stageId: string) =>
      jobsService.updateStage(jobId, stageId, {
        status: "completed",
  ...
  const deleteStage = useMutation({
    mutationFn: (stageId: string) => jobsService.deleteStage(jobId, stageId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["job-stages", jobId] });
      queryClient.invalidateQueries({ queryKey: ["job", jobId] });
    ...
    onError: () => {
      showErrorNotification(t("jobs.deleteStageError"));
    },
  });
  ...
  const handleChangeStatus = (stage: JobStageDTO) => {
    setSelectedStage(stage);
    setMenuOpen(null);
  };

  const handleDeleteClick = (stage: JobStageDTO) => {
  ...
  return (
    <div className="relative space-y-6">
      ...
      {timelineItems.map((item, index) => {
        ...
        const stage = item.data as JobStageDTO;
                ...
                <div className="flex gap-2">
                  ...
                  <div className="relative">
                    <Button
                      ...
                      aria-label={t("jobs.stageOptions")}
                    >
      ...
      >
        <DialogContent onClose={() => setConfirmDeleteStage(null)}>
          <DialogHeader>
            <DialogTitle>{t("jobs.deleteStage")}</DialogTitle>
            <DialogDescription>
              {confirmDeleteStage &&
                t("jobs.deleteStageConfirm", {
